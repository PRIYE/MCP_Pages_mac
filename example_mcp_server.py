...
import win32con
import time
from win32con import GetSystemMetrics
import subprocess
import time
...
async def draw_rectangle_mac(x1: int, y1: int, x2: int, y2: int) -> dict:
    """Draw a rectangle in Paint S from (x1,y1) to (x2,y2)"""
    try:
        # AppleScript to draw rectangle in Paint S
        script = f'''
        tell application "Paint S"
            activate
            delay 1
            tell application "System Events"
                keystroke "r" -- Select rectangle tool
                delay 0.5
                set position of mouse to {{{x1}, {y1}}}
                delay 0.2
                click
...
async def draw_rectangle_and_text_mac(text: str) -> dict:
    """Draw a rectangle and add text in Paint S"""
    try:
        # AppleScript to draw rectangle and add text in Paint S
        script = f'''
        tell application "Paint S"
            activate
            delay 1
            tell application "System Events"
                ...
                keystroke "r"
                delay 0.5
                set position of mouse to {{100, 100}}
                click
                drag from {{100, 100}} to {{300, 200}}
                
                -- Switch to text tool and add text
                keystroke "t"
                delay 0.5
                set position of mouse to {{150, 150}}
                click
                delay 0.2
                keystroke "{text}"
            end tell
    ...
    try:
        ...
        return {
            "content": [
                TextContent(
                    type="text",
                    text="Paint S opened successfully"
                )
            ]
        }
    except Exception as e:
        return {
            "content": [
                TextContent(
                    type="text",
                    text=f"Error opening Paint S: {str(e)}"
                )
            ]
        }     
